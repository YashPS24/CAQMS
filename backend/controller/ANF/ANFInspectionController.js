...
export const getAnfOrderDetails = async (req, res) => {
  try {
    const { moNo } = req.params;
    // Querying the dt_orders collection using the caProdConnection
    ...
    order.OrderColors.forEach((colorObj) => {
      ...
      // FIXED: Handle the OrderQty array structure correctly
      colorObj.OrderQty.forEach((sizeEntry) => {
        // Each sizeEntry is an object like { "XS": 283 } or { "S": 493 }
        Object.keys(sizeEntry).forEach((sizeKey) => {
          // Skip the MongoDB _id field
          if (sizeKey !== '_id') {
            const quantity = sizeEntry[sizeKey];
            if (quantity > 0) {
              colorQtyBySize[color][sizeKey] = quantity;
            }
          }
        });
      });
