...
async function connectPool(pool, poolName) {
  try {
    ...
    await pool.connect();
    console.log(`✅ Successfully connected to ${poolName} pool at ${pool.config.server}`);
  ...
  try {
    ...
    const orderHeaderQuery = `
      ...
      WHERE h.[Order_No] IS NOT NULL 
        AND (h.[Factory] IS NULL OR h.[Factory] <> 'YM')
      ORDER BY h.[Order_No]
    `;
    ...
    const validOrderNos = orderHeaderResult.recordset.map(header => header.Order_No);
    ...
    for (const [colorKey, colorSummary] of colorSummaryMap) {
      ...
      if (orderQtyArray.length > 0) { // Only add colors with actual quantities
...
export const syncDtOrders = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: "DT Orders data sync completed successfully (excluding YM factory, including shipping fields)",
      data: result
    });
  } catch (error) {
...
cron.schedule("0 */3 * * *", async () => {
  console.log("⏰ Running scheduled DT Orders sync (excluding YM factory, including shipping fields)...");
  await syncDTOrdersData()
    .then((result) => {
